...
def main():
    ...
    assert not (args.pdb is not None and args.select_length) # pdb 和select length 有一个存在
    ...
    for _ in tqdm(range(args.n_iter)):
        ...
        sample, n = sampling_fn(state["model"], condition) # sample_fn(state model condition)
